    ...
    <table class="w-full table-auto border-collapse rounded-md">
        ...
        <tr class="odd:bg-[#FCF9F4] even:bg-[#E6EED5]">
            ...
            <td class="border-2 border-[#D2DAC2] flex flex-col space-x-0 space-y-4 lg:space-x-4 lg:space-y-0 lg:flex-row px-10 lg:text-xs text-3xl py-10 lg:py-4">
                <a href="detailSchool/{{ $sch['id'] }}"  class=" border-2 border-[#9CB668] hover:bg-white hover:text-[#9CB668] rounded-[5px] text-center px-5 p-2 bg-[#9CB668] text-white transition duration-200">View</a>
                ...
                <form action="{{ url('schools/' . $sch->id) }}" method="POST" class="flex items-center justify-center mt-4 ml-5">
                    @csrf
                    @method('DELETE')
                    <button class="border-2 border-[#E42029] hover:text-[#E42029] hover:bg-white rounded-[5px] px-5 p-2 bg-[#E42029] text-white transition duration-200">Delete</button>
                </form>
